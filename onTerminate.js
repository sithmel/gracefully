...
function onTerminate (shutdownFunction, options) {
  const {logger, customEvent, handleExceptions, stopWindow, exitDelay} = {...DEFAULTS, ...options}
  ...
  function shutDown (reason, code) {
    ...
    const timeoutFunction = timeoutMessage(stopWindow, `Graceful shutdown took more than stop window (${stopWindow} ms). Terminating process.`)
